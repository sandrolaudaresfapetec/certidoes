...
# Apply migrations using node + better-sqlite3 directly
node -e "
...
const migrationsDir = path.join('/app/prisma/migrations');
const dirs = fs.readdirSync(migrationsDir).filter(d => {
  const stat = fs.statSync(path.join(migrationsDir, d));
  return stat.isDirectory() && d !== 'migration_lock.toml';
}).sort();
...
for (const dir of dirs) {
  ...
  db.exec(sql);
  db.prepare('INSERT INTO _prisma_migrations (id, checksum, finished_at, migration_name, applied_steps_count) VALUES (?, ?, datetime(\"now\"), ?, 1)').run(
